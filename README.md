...
A session cookie is `HttpOnly` and `SameSite=Lax`. On HTTPS it is also `Secure`. The published host does not keep that cookie, so this page also holds the sign-in itself and sends the account with each request. Continue with Google opens the account page in another window. This page keeps checking, then closes that window and opens the desk. You do not switch back. It says sign-in expired only when the device code itself has ended. Sign out on the desk ends a normal login. A desk that is already open is also kept in `data/sessions.json` (mode `0600`, gitignored) on this machine.
