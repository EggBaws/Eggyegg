...
A session cookie is `HttpOnly` and `SameSite=Lax`. On HTTPS it is also `Secure`. The cookie holds a random id, not the email and not the exchange keys. A desk that is already open is kept in `data/sessions.json` (mode `0600`, gitignored) so a restart of this machine can open it again. The waiting Google login stays in the browser as its own `HttpOnly` cookie. Leave this tab open, authorise the device, and come back: this page polls and opens the desk. It says sign-in expired only when the device code itself has ended. Sign out on the desk ends a normal login.
